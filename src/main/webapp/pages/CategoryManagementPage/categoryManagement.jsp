...
<div class="bg-white rounded-xl shadow-lg p-5">
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="w-full text-sm text-left text-gray-600">
            ...
            <tbody id="categoryTableBody" class="divide-y divide-gray-200">
                <c:forEach items="${listdata}" var="cat">
                    <tr class="category-row hover:bg-gray-50 transition-colors" data-status="${cat.isActive ? 'active' : 'inactive'}">
                        <td class="px-4 py-3 text-center font-medium text-gray-500">#${cat.categoryId}</td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-4">
                                <%-- Action icons đồng bộ với Product --%>
                                <a href="categoryServlet?action=detail&id=${cat.categoryId}" class="text-blue-500 hover:text-blue-700 transition-transform hover:scale-110">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="categoryServlet?action=edit&id=${cat.categoryId}" class="text-orange-500 hover:text-orange-700 transition-transform hover:scale-110">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <a href="categoryServlet?action=delete&id=${cat.categoryId}" class="text-red-500 hover:text-red-700 transition-transform hover:scale-110">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <c:if test="${empty listdata}">
            <div id="noDataRow" class="px-4 py-10 text-center text-gray-400 italic bg-gray-50">No categories found.</div>
        </c:if>
    </div>

    <%-- 5. PHÂN TRANG (Pagination style Product) --%>
    <div class="flex flex-col md:flex-row justify-between items-center gap-3 mt-5 text-sm text-gray-500">
        <p>Showing <span id="visibleCount" class="font-bold text-gray-900">${listdata.size()}</span> categories</p>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function filterTable() {
            ...
            // Cập nhật text số lượng hiển thị
