        ...
        <div class="bg-white border border-gray-100 rounded-[2rem] overflow-hidden shadow-sm mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 p-8 border-b border-gray-100 bg-gray-50/30">
                <div class="text-center space-y-2">
                    <div class="text-6xl font-black text-red-600 leading-none"><fmt:formatNumber value="${averageRating}" maxFractionDigits="1" minFractionDigits="1" /></div>
                    <div class="text-yellow-400 text-2xl">
                        <c:forEach begin="1" end="5" var="i"><c:choose><c:when test="${i <= averageRating}">★</c:when><c:otherwise><span class="text-gray-200">★</span></c:otherwise></c:choose></c:forEach>
                                </div>
                ...
                <div class="space-y-2">
                    <c:forEach begin="1" end="5" step="1" var="star">
                        ...
                        <div class="flex items-center gap-3 text-sm">
                            ...
                            <div class="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden"><div class="h-full bg-red-600 rounded-full" style="width: ${starPercentages[idx]}%"></div></div>
                ...
                <div class="flex items-center justify-center">
                    <c:choose>
                        <c:when test="${userReview != null}"><button onclick="openReviewModal(true)" class="px-6 py-3 bg-white border-2 border-blue-600 text-blue-600 font-black rounded-2xl hover:bg-blue-600 hover:text-white transition-all text-xs">✍️ EDIT YOUR REVIEW</button></c:when>
                        <c:when test="${canReview}"><button onclick="openReviewModal(false)" class="px-6 py-3 bg-red-600 text-white font-black rounded-2xl hover:bg-red-700 transition-all shadow-md text-xs">✍️ WRITE A REVIEW</button></c:when>
                        <c:otherwise><p class="text-xs text-gray-400 font-bold italic text-center max-w-[150px]">Must purchase this product to leave a review.</p></c:otherwise>
                    </c:choose>
                </div>
            </div>
            <div class="divide-y divide-gray-50 px-8">
                <c:choose>
                    <c:when test="${empty productReviews}"><div class="py-16 text-center text-gray-400 italic">No reviews yet.</div></c:when>
                    <c:otherwise>
                        <c:forEach var="rev" items="${productReviews}">
                            <div class="py-8 flex gap-6"><div class="w-12 h-12 bg-red-50 text-red-600 font-black rounded-2xl flex items-center justify-center text-xl flex-shrink-0">${fn:substring(rev.customerName, 0, 1)}</div>
                                <div class="flex-1"><div class="flex justify-between items-center mb-1"><h4 class="font-black text-gray-900">${rev.customerName}</h4><span class="text-xs font-bold text-gray-300 uppercase">${rev.formattedDate}</span></div>
                                    <div class="text-yellow-400 text-xs mb-2"><c:forEach begin="1" end="5" var="i"><c:choose><c:when test="${i <= rev.rating}">★</c:when><c:otherwise><span class="text-gray-100">★</span></c:otherwise></c:choose></c:forEach></div>
                                    <p class="text-gray-600 text-sm leading-relaxed">${rev.comment}</p></div></div>
                                </c:forEach>
                            </c:otherwise>
                        </c:choose>
            </div>
...
<%-- MODAL REVIEW --%>
<div id="reviewModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300">
        ...
        <form action="${pageContext.request.contextPath}/reviewservlet" method="post" id="reviewForm">
            ...
            <div class="text-center mb-6">
                <div class="flex justify-center gap-2 mb-2">
                    <c:forEach begin="1" end="5" var="i"><button type="button" onclick="setRating(${i})" class="star-btn text-3xl text-gray-200 transition-colors" data-value="${i}">★</button></c:forEach>
                    </div>
            ...
            <div class="flex gap-3">
                <button type="button" onclick="closeReviewModal()" class="flex-1 h-12 rounded-xl border border-gray-200 font-bold text-gray-500 hover:bg-gray-50">Cancel</button>
                <button type="submit" class="flex-1 h-12 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 shadow-md">Submit Review</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    <c:forEach var="vs" items="${variantSpecs}">
                    ...
                    function openReviewModal(isEdit) {
                        ...
                        setRating(document.getElementById('reviewRating').value || 5);
                    }
                    ...
                    function setRating(rating) {
                        ...
                        document.querySelectorAll('.star-btn').forEach(s => s.classList.toggle('active', parseInt(s.dataset.value) <= rating));
                        var texts = ["Poor", "Dissatisfied", "Average", "Satisfied", "Excellent"], colors = ["#6b7280", "#f97316", "#3b82f6", "#16a34a", "#dc2626"];
