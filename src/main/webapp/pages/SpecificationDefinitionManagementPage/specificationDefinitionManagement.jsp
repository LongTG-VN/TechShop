...
<div class="bg-white rounded-xl shadow-lg p-5">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold text-gray-800 uppercase tracking-tight">Specification Definitions</h2>
        <a href="specificationServlet?action=add" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all flex items-center gap-2 shadow-md">
    ...
    <div class="flex flex-col md:flex-row gap-3 mb-6 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
        <div class="relative flex-1 w-full">
            <input type="text" id="specNameFilter" oninput="filterSpecs()" placeholder="Search specification name..." 
        ...
        <select id="catFilter" onchange="filterSpecs()" class="w-full md:w-64 px-3 py-2 text-sm border border-gray-300 rounded-lg outline-none bg-white cursor-pointer shadow-sm">
        ...
        <table class="w-full text-sm text-left text-gray-600">
            <thead class="text-xs uppercase bg-gray-50 text-gray-700 font-bold">
                <tr>
                    ...
                    <th class="px-4 py-3">Spec Name</th>
            ...
            <tbody id="specTableBody" class="divide-y divide-gray-200">
                <c:forEach items="${listdata}" var="s">
                    <tr class="spec-row hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-center font-medium text-gray-500">#${s.specId}</td>
                        <td class="px-4 py-3 font-bold text-gray-900 spec-name-cell">${s.specName}</td>
                        <td class="px-4 py-3 cat-name-cell">${s.categoryName}</td>
                        <td class="px-4 py-3 text-gray-500">${not empty s.unit ? s.unit : '-'}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-bold border ${s.isActive ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}">
                                ${s.isActive ? 'ACTIVE' : 'INACTIVE'}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center gap-4">
                                <a href="specificationServlet?action=detail&id=${s.specId}" class="text-blue-500 hover:scale-110 transition-transform" title="View Detail">
                                ...
                                <a href="specificationServlet?action=edit&id=${s.specId}" class="text-orange-500 hover:scale-110 transition-transform" title="Edit">
                                ...
                                <a href="specificationServlet?action=delete&id=${s.specId}" class="text-red-500 hover:scale-110 transition-transform" title="Delete">
                ...
                <tr id="noResults" style="display: none;">
                    <td colspan="6" class="py-12 text-center text-gray-400 italic bg-gray-50 rounded-b-lg">
                        <div class="flex flex-col items-center">
                            ...
                            <span>No specifications match your search or filter.</span>
                        </div>
                    </td>
                </tr>
            </tbody>
...
<script>
    function filterSpecs() {
        const nameVal = document.getElementById('specNameFilter').value.toLowerCase();
        const catVal = document.getElementById('catFilter').value.toLowerCase();
        const rows = document.querySelectorAll('.spec-row');
        let count = 0;

        rows.forEach(row => {
            const name = row.querySelector('.spec-name-cell').innerText.toLowerCase();
            const cat = row.querySelector('.cat-name-cell').innerText.toLowerCase();
            const isVisible = name.includes(nameVal) && (catVal === "" || cat === catVal);
